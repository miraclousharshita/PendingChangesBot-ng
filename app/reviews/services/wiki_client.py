...
class WikiClient:
    ...
    def fetch_pending_pages(self, limit: int = 10000) -> list[PendingPage]:
        ...
        configuration = getattr(self.wiki, "configuration", None)
        use_test_mode = configuration and configuration.test_mode

        if use_test_mode:
            # Get revision IDs from configuration
            test_ids = configuration.test_revision_ids or []
            ...
            # Build TEST SQL query with fixed timestamp
            sql_query = f"""
SELECT
   page_title,
   page_namespace,
