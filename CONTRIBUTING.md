...
It is advised to follow the below mentioned pointers when trying to contact the mentors:
...
- **[zache-fi](https://github.com/zache-fi)**: zache on Slack (Bot's original developer in 2016 and maintainer. Mentoring Cat-a-lot Outreachy in round 30)
...
- **Ipr1**: Finnish Wikipedist and Wikimedia Finland's fiwiki developer/helpdesk person
...
Pywikibot needs to log in to [meta.wikimedia.org](https://meta.wikimedia.org) and approve Superset's OAuth client before the SQL queries in `SupersetQuery` will succeed. Follow the steps below once per user account that will run PendingChangesBot:
...
- **Test branches** (test added or updated): `test/issueNumber`

Notice how each branch refers to an issue number. This is essential to keep track of which issue is being worked on and to ease collaboration. It's good practice to have one feature or fix per branch. Keep commits focused and atomic, avoid putting unrelated changes in one branch.

### Commit Messages

> **Security Note:** If the app needs auth tokens, URLs or secrets then be sure to remove them or include in `.gitignore` before making commits.
...
- **mypy**: Static type checker for Python (gradual adoption in progress)
- **pre-commit**: Runs checks automatically before commits
- **pip-audit**: Scans for security vulnerabilities in dependencies

### Type Checking (Gradual Adoption)

The project is gradually adopting type hints:

- Configuration is in `pyproject.toml` under `[tool.mypy]`
- Currently using lenient settings to allow gradual adoption
- Type hints are encouraged but not required for all new code
- Focus on typing public APIs and critical functions first

**Running mypy locally**:
```bash
cd app
python -m mypy reviews --config-file=../pyproject.toml
```

### Testing Guidelines

- Write tests for new features and bug fixes
- Run tests locally before pushing: `python manage.py test`
- Aim for meaningful test coverage, not just 100% coverage
...
- Minimize N+1 query problems with `select_related()` and `prefetch_related()`
...
- Run security checks: `python -m ruff check --select S app/`
...
│   ├── reviews/                  # Core application
│   │   ├── models/              # Database models
│   │   ├── services/            # External API clients
│   │   ├── autoreview/          # Automated review logic
│   │   ├── views.py             # API endpoints
│   │   └── tests/               # Test files
│   ├── reviewer/                # Django project settings
│   └── static/                  # Static files (Vue.js frontend)
